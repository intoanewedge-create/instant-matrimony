...
function LoginForm() {
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      if (res?.error) {
        if (res.error.includes("EMAIL_UNVERIFIED") || res.error.includes("unverified")) {
          setError(
            <span>
              Your email is not verified.{" "}
              <Link href={`/verify-email?email=${encodeURIComponent(data.email)}`} className="text-rose-400 underline font-semibold">
                Click here to verify.
              </Link>
            </span>
          );
        } else {
          setError("Invalid email or password. (Default test password: User@123 or Admin@123)");
        }
      } else {
